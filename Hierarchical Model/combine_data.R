...
one_id = function(one, cond) {
  ...
  std_alternations = sum(one_standard$text_image, one_standard$text_ws, 
                         one_standard$text_dash, na.rm = TRUE)
  ...
  cond_alternations = sum(one_other$text_image, one_other$text_ws, 
...
# mediation model using fix_text_pct

# not sig
md_E_med_1 = lmer(E ~ scale(fix_text_pct) + (1|id), data = data_new)
summary(md_E_med_1)

# not sig
md_Q_med_1 = lmer(Q ~ scale(fix_text_pct) + (1|id), data = data_new)
summary(md_Q_med_1)




# mediation model using fix_image_pct

# not sig
md_E_med_2 = lmer(E ~ scale(fix_image_pct) + (1|id), data = data_new)
summary(md_E_med_2)

# not sig
md_Q_med_2 = lmer(Q ~ scale(fix_image_pct) + (1|id), data = data_new)
summary(md_Q_med_2)




# mediation model using fix_text_pct - fix_image_pct
# not sig
md_E_med_3 = lmer(E ~ scale(fix_text_pct - fix_image_pct) + (1|id), data = data_new)
summary(md_E_med_3)

# not sig
md_Q_med_3 = lmer(Q ~ scale(fix_text_pct - fix_image_pct) + (1|id), data = data_new)
summary(md_Q_med_3)


# mediation model using fix_text_pct + fix_image_pct
# not sig
md_E_med_4 = lmer(E ~ scale(fix_text_pct + fix_image_pct) + (1|id), data = data_new)
summary(md_E_med_4)

# not sig
md_Q_med_4 = lmer(Q ~ scale(fix_text_pct + fix_image_pct) + (1|id), data = data_new)
summary(md_Q_med_4)


# mediation model using text_alternation with text_alternation = text -> all
# sig
md_E_med_4 = lmer(E ~ scale(text_alternations) + (1|id), data = data_new)
summary(md_E_med_4)

# not sig
md_med_4 = lmer(text_alternations ~ relevel(condition, "standard") + (1|id), data = data_new)
summary(md_med_4)


# not sig
md_Q_med_4 = lmer(Q ~ scale(text_alternations) + (1|id), data = data_new)
summary(md_Q_med_4)
...
pca_try = prcomp(data_new[, c(6, 7, 8)])
# mediation model w/ pca for E
# not sig
