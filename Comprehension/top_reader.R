...
#' April, 2018
#'
#' Create a dataframe consists of the good reader in each trial 
#' and each condition. The good reader is defined by the five  
#' participants with the highest retelling score.
...
top_standard = top_n(standard_retell, n = 5, wt = Pct) %>%
                arrange(by = desc(Pct)) %>%
...
top_separated = top_n(separated_retell, n = 5, wt = Pct) %>%
                  arrange(by = desc(Pct)) %>%
...
top_partial = top_n(partial_retell, n = 5, wt = Pct) %>%
                    arrange(by = desc(Pct)) %>%
...
top_clean = top_n(clean_retell, n = 5, wt = Pct) %>%
                  arrange(by = desc(Pct)) %>%
...
# join all the top reteller together
top_reteller = do.call(rbind, list(top_standard, top_separated, top_partial, top_clean))
write.csv(top_reteller, 
          file = "~/Desktop/36726 Statistical Practice/Project/data/Cluster/Top Retell/top_retller.csv",
          row.names = FALSE)
